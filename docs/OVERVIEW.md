...
- **CIゲート（本番に出る前）**：PR時にオフライン評価を実行し、**品質が下がる変更はマージをブロック**。groundedness を **Gemini judge** でスコア化（`aegis_platform.eval.gate`・Vertex Gen AI Eval pointwiseメトリクス互換）。
...
> CI品質ゲート（groundedness）は **Gemini judge で実装済み**（`uv run python -m aegis_platform.eval.gate`・cloud-mode／"無害な"プロンプト改変で根拠性が落ちるのを実LLMで検知しマージをブロック）。Vertex Gen AI Eval pointwiseメトリクスへの差し替え、PII/指示遵守judgeは継続項目。
