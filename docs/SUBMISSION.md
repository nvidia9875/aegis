...
- **画像**: `aegis-clear-idle.png`（透過ホログラムUI・待機）, `aegis-clear-active.png`（インシデント対応中・赤アラート＋ワイヤーフレーム封入＋多色プラズマコア）, `aegis-hud-02-gate.png`（Governanceゲート）（three.js/R3F製。**Passengers風の透過ホロUI**：レイヤー重ねの半透明ガラスパネル＋色収差フリンジ＋中央ワイヤーフレーム・ホログラム＋ボリュメトリック・ヘイズ＋屈折ガラス多色プラズマコア＋反射床＋bloom/DoF/HDRグレーディング。周辺パネルも全てWebGL内で実CIデータのみ）
