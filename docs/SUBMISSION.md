...
- **画像**: `aegis-glass-idle.png`（3Dシーン・待機）, `aegis-glass-active.png`（インシデント対応中・ガラス封入の赤プラズマ＋エネルギービーム＋実ループ可視化）, `aegis-hud-02-gate.png`（Governanceゲート）（three.js/R3F製。**屈折ガラスシェル＋GLSLプラズマ・コア（3D simplexノイズ＋ドメインワープ）＋カール風シェーダ粒子コロナ**・反射床・星屑・bloom/DoF/色収差/HDRグレーディング。周辺パネルも全てWebGL内で実CIデータのみ）
