...
- **画像**: `aegis-glasshud-idle.png`（透過ホログラムUI・待機）, `aegis-glasshud-active.png`（インシデント対応中・赤アラート透過パネル＋多色プラズマコア）, `aegis-hud-02-gate.png`（Governanceゲート）（three.js/R3F製。**透過ホログラフィック・ガラスパネル**＋屈折ガラスシェル＋多色GLSLプラズマコア＋反射床＋bloom/DoF/色収差/HDRグレーディング。周辺パネルも全てWebGL内で実CIデータのみ）
