...
- **画像**: `aegis-3d-tuned.png`（3Dシーン全体・待機）, `aegis-3d-full-active.png`（インシデント対応中・実ループ可視化）, `aegis-hud-02-gate.png`（Governanceゲート）（リポジトリ直下。three.js/R3F製の3Dシーン。周辺パネルも全てWebGL内で実CIデータのみを可視化）
