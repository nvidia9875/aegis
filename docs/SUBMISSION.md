...
- **画像**: `aegis-plasma-idle.png`（3Dシーン・待機）, `aegis-plasma-active.png`（インシデント対応中・コア赤プラズマ＋エネルギービーム＋実ループ可視化）, `aegis-hud-02-gate.png`（Governanceゲート）（three.js/R3F製。**GLSLプラズマ・コア（3D simplexノイズ＋ドメインワープ）**・粒子コロナ・反射床・星屑・bloom/DoF/色収差。周辺パネルも全てWebGL内で実CIデータのみ）
